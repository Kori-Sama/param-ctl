...
def main():
    ...
    pid = PIDController(
        kp=pm.get("kp"), ki=pm.get("ki"), kd=pm.get("kd"), setpoint=pm.get("setpoint")
    )
    system = SimpleSystem(
        initial_value=0.0,
    ...
    try:
        ...
        while True:
            ...
            plt.legend()
            plt.title(
                f"PID Control (kp={pid.kp:.2f}, ki={pid.ki:.2f}, kd={pid.kd:.2f})"
            )
            ...
            print(
                f"\rTime: {current_time:.1f}s | Setpoint: {pid.setpoint:.2f} | Value: {system_value:.2f} | Error: {pid.setpoint - system_value:.2f} | Control: {control_signal:.2f}",
                end="",
            )
